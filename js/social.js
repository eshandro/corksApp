...
function parseMap(data, store) {
	if (data.meta.code === 200) {
		if (store) {
			myLocalStorage.set('foursquarePlaces', data);
			var currentTime = new Date().getTime();
			myLocalStorage.set('lastFoursquarePull', currentTime);
		}
...
function findPlaces() {
	// See if places data exists in local storage
	var placesData = myLocalStorage.get('foursquarePlaces', 'JSON');
	var lastTime = myLocalStorage.get('lastFoursquarePull');

	if (lastTime) {
		var currentTime = new Date().getTime();
		var timeDiff = parseInt(lastTime) - parseInt(currentTime);
		if (timeDiff > 250) {
